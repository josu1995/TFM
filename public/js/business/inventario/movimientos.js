...
const initPopovers = () => {
  ...
  function removeFromChecked(val) {
    ...
    if(cont == 1){
      ...
      var value = $('#tipo'+id).text();
      if(value == 'Edición'){
        $('#cancelarEdicion').show();
        $('#cancelarEdicionSeparator').show();
      }
    }else{
      $('#cancelarEdicionSeparator').hide();
    ...
    $('.table-checkbox').on('ifChecked', function (event) {
     ...
     var pages = getPages();
      ...
      if(cont == 1){
        ...
        var value = $('#tipo'+checkValue).text();
        console.log(value);
        if(value == 'Edición'){
          $('#cancelarEdicion').show();
          $('#cancelarEdicionSeparator').show();
  ...
  $(function () {
    ...
    $('#cancelarEdicion').click(function (e) {
      e.preventDefault();
      console.log('e');
      var id = 0;
      for(var i = 0;i<=checkedRows.length;i++){
      ...
      $.ajax({
        ...
        error: function (data) {
          ...
          keys.forEach(function (val, i) {
            var row = parseInt(val.split('.')[1]) + 1;
            var plainRow = row - 2;
